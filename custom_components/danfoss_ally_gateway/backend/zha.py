...
class ZHABackend(DanfossBackend):
    ...
    mapping from UUID to ZHA climate entity ID for service calls.
    ...
    async def async_resolve_trv_identifier(self, device_registry_id: str) -> str | None:
        """Resolve a device registry ID to a ZHA climate entity ID.

        Looks up the device in the device registry, then finds the climate
        entity associated with it via the entity registry.
        Returns None if the device or entity cannot be found.
        """
        ...
        entries = er.async_entries_for_device(entity_reg, device_registry_id)
        for entry in entries:
            if entry.domain == "climate":
                return entry.entity_id
        return None

    def _resolve_entity(self, trv_id: str) -> str:
        """Resolve a TRV UUID to its ZHA climate entity ID.

        Falls back to using the trv_id as-is if no mapping exists
        (backwards compatibility with test fixtures).
        """
        return self._id_to_entity.get(trv_id, trv_id)
    ...
    def _handle_zha_gateway_message(self, data: dict) -> None:
        ...
        if entity_id is not None:
            # Reverse-lookup: entity_id -> UUID
            trv_uuid = self._entity_to_id.get(entity_id)
            if trv_uuid and trv_uuid in self._subscriptions:
                _LOGGER.debug(
                    "Device announce for subscribed TRV: %s (UUID: %s, ieee=%s)",
                    entity_id,
                    trv_uuid,
                    ieee,
                )
    ...
    def _find_entity_by_ieee(self, ieee: str) -> str | None:
        """Find the ZHA climate entity_id for a given IEEE address."""
    ...
    async def async_subscribe_trv(self, trv_id: str) -> None:
        ...
        Args:
            trv_id: Device registry UUID (or legacy entity_id).
        """
        if trv_id in self._subscriptions:
            return

        # Resolve UUID -> entity_id for ZHA state tracking
        entity_id = self._id_to_entity.get(trv_id)
        if entity_id is None:
            resolved = await self.async_resolve_trv_identifier(trv_id)
            if resolved is not None:
                entity_id = resolved
                self._id_to_entity[trv_id] = entity_id
                self._entity_to_id[entity_id] = trv_id
            else:
                # Fallback: treat trv_id as the entity_id itself
                # (backwards compat for test fixtures)
                entity_id = trv_id
    ...
    async def async_unsubscribe_trv(self, trv_id: str) -> None:
        ...
        if entity_id is not None:
            self._entity_to_id.pop(entity_id, None)
